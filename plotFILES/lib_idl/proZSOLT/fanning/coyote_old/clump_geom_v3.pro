...
;SPHERICAL CLUMP 
;ATTEMPT TO USE ONVLY VECTORS 
Xo = 1.5
Yo = 1.0
Zo = 3.0
Roff = [Xo,Yo,Zo]
;Takes out center of spherical clump 
R1 = 0.5 
;TRY AND DEFINE ON CART. GRIDS DIRECTLY
X1 = findgen(nb)/(nb-1.)*2.*(R1+Xo) - Xo
Y1 = findgen(nb)/(nb-1.)*2.*(R1+Yo) - Yo
Z1 = FLTARR(NB,NB)
FOR I=0,NB-1 DO Z1(*,I) = SQRT(R1^2.-(X1-Xo)^2.-(Y1(I)-Yo)^2.) + Zo
;Z-values 
...
yr=xr
zr=zr
;SURFACE, DIST(5),/NODATA, /SAVE, XR=xr, YR=yr, ZR=zr, $
;         Xs=st, Ys=st, Zs=st, az=az, ax=ax

SURFACE, Z1,X1,Y1, /SAVE
;SURFACE, Z1-2.*(Z1-Zo),X1,Y1, /SAVE, /NOERASE
save
message,'testing..'
...
;oplot,[v11(0),v22(0),v33(0),v44(0),v11(0)],[v11(1),v22(1),v33(1),v44(1),v11(1)],thick=tsh
...
PRO SPH_to_CART,X,Y,Z,R,TH,PHI
;Transforms sph. to cart. 
X = R*sin(TH)*cos(PHI)
Y = R*sin(TH)*sin(PHI)
Z = R*cos(TH)
end 
