...
; CALLING SEQUENCE:
;       LEGEND_VGDL [,items][,keyword options]
; EXAMPLES:
;       The call:
;               legend_vgdl,['Plus sign','Asterisk','Period'],psym=[1,2,3]
;         produces:
;               -----------------
;               |               |
;               |  + Plus sign  |
...
;       legend_vgdl,items,linestyle=lines            ; vertical legend---upper left
;       items = ['Plus sign','Asterisk','Period']
;       sym = [1,2,3]
;       legend_vgdl,items,psym=sym                   ; ditto except using symbols
;       legend_vgdl,items,psym=sym,/horizontal       ; horizontal format
;       legend_vgdl,items,psym=sym,box=0             ; sans border
;       legend_vgdl,items,psym=sym,delimiter='='     ; embed '=' betw psym & text
;       legend_vgdl,items,psym=sym,margin=2          ; 2-character margin
;       legend_vgdl,items,psym=sym,position=[x,y]    ; upper left in data coords
;       legend_vgdl,items,psym=sym,pos=[x,y],/norm   ; upper left in normal coords
;       legend_vgdl,items,psym=sym,pos=[x,y],/device ; upper left in device coords
;       legend_vgdl,items,psym=sym,/position         ; interactive position
;       legend_vgdl,items,psym=sym,/right            ; at upper right
;       legend_vgdl,items,psym=sym,/bottom           ; at lower left
;       legend_vgdl,items,psym=sym,/center           ; approximately near center
;       legend_vgdl,items,psym=sym,number=2          ; plot two symbols, not one
;       legend_vgdl,items,/fill,psym=[8,8,8],colors=[10,20,30]; 3 filled squares
; INPUTS:
...
;           c2_line = [0,2,1]
;           legend_vgdl,c1_items,psym=c1_psym,corners=c1,box=0
;           legend_vgdl,c2_items,line=c2_line,corners=c2,box=0,pos=[c1[2],c1[3]]
...
;           legend_vgdl,['1','22','333','4444'],linestyle=indgen(4),corners=corners
...
;           legend_vgdl,['1','22','333','4444'],linestyle=indgen(4),pos=pos
...
;-
pro legend_vgdl, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
...
;
on_error,2
if keyword_set(help) then begin & doc_library,'legend_vgdl' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type legend_vgdl,/help.'
if ni eq 0 then begin
  items = replicate('',n)                               ; DEFAULT BLANK ARRAY
endif else begin
  if size(items,/TNAME) NE 'STRING' then message, $
      'First parameter must be a string array.  For help, type legend_vgdl,/help.'
...
case n_elements(position) of
 ...
 else: message,'Position keyword can have 0, 1, or 2 elements only. Try legend_vgdl,/help.'
endcase
