PRO getFLUXEM_FORSOL_NEW, dir, name_indx, FILE_EXIST, NXOBS, ALPHA, GAMMA, XOBS, FLUX_TOT, FLUX_CONT, file_name=file_name, help=print_help, conv=conv
;+
; NAME:
;       getFLUXEM_FORSOL_NEW
;
; PURPOSE:
;       This procedure reads in emergent flux profiles from output-files
;
; CALLING SEQUENCE:
;       getFLUXEM_FORSOL_NEW, dir, name_indx, file_exist, nxobs, alpha, gamma, $
;       xobs, flux_tot, flux_cont
;
; INPUTS:
...
;       conv:   Set this keyword (flag) to read files according to output
;               from convolution procedure.
;      
; OUTPUTS:
...
;
; EXAMPLE:
;       getFLUXEM_FORSOL_NEW, '.', 0, f_exist, 100, alpha, gamma, xobs, ftot,
;       fcont
;       getFLUXEM_FORSOL_NEW, '.', 10, f_exist, 100, alpha, gamma, lamb, ftot,
;       fcont, /CONV
;       getFLUXEM_FORSOL_NEW, '.', 10, f_exist, 100, alpha, gamma, xobs, ftot,
;       fcont, file_name='unusual_output_name.dat'
;- 
;-------------------------OUTPUT IF HELP NEEDED-------------------------

if(keyword_set(print_help)) then begin 
 doc_library, 'getfluxem_forsol_new'
 return
endif
;
;-----------------------DEFINE FILE-NAMES-------------------------------
;
IF(name_indx eq 0) THEN BEGIN
   PRINT, 'if fname-keyword set, set additionally xobs_indx ne 0'
   ;call old read-procedure (for alpha, gamma=0.)
   getFLUXEM_FORSOL, dir, FILE_EXIST, NXOBS, XOBS, FLUX_TOT, FLUX_CONT
   ALPHA=0.d0
   GAMMA=0.d0
   RETURN
ENDIF
...
ENDIF ELSE BEGIN
   ...
   fname=dir+'/FLUXEM_'+dummy_name+'.dat'
ENDELSE
;
...
ENDIF
;
;----------READ IN CONVOLVED OUTPUT STORAGE STYLE IF FLAG IS SET--------
;
IF(KEYWORD_SET(CONV)) THEN BEGIN
   readcol, fname, xobs, flux_tot
;set flux_cont to 1, since flux_tot already normalized
   flux_cont=flux_tot/flux_tot 
   nxobs=n_elements(xobs)
;set alpha gamma to dummy-values
   alpha=0.d0
   gamma=0.d0
   RETURN
ENDIF
