PRO plotFLUXEM, dir, fname=FNAME, obs_indx=OBS_INDX, oname=ONAME, windx=WINDX, ylim=YLIM, xlim=XLIM, oplot=OPLOT, ls=LS, cindx=CINDX, freq=FREQ, wave=WAVE, vel=VEL, lam0=LAM0, vth_fid=VTH_FID, xscalefac=xscalefac, isotropic=ISOTROPIC, charsize=charsize, help=print_help
;
;+
; NAME:
;	plotFLUXEM
...
;
;       ls:     Set this keyword to an integer, describing the linestyle, with
;               which the emergent profile is plotted
;
...
IF(KEYWORD_SET(print_help)) THEN BEGIN
   doc_library, 'plotFLUXEM'
   return
ENDIF
;
IF (N_ELEMENTS(dir) EQ 0) THEN BEGIN
   PRINT, 'SYNTAX: '
   PRINT, 'plotFLUXEM, dir'
   PRINT, 'with dir=string , e.g. plotFLUXEM, "M1" '
   RETURN
ENDIF
;
...
;-----------------------------------------------------------------------
;
getFLUXEM_FORSOL_NEW, dir, obs_indx, FILE_EXIST, NXOBS, ALPHA, GAMMA, XOBS, FLUX_TOT, FLUX_CONT, file_name=fname
;
;-----------------------------------------------------------------------
;
...
;   ylim=[MIN(FLUX_TOT/FLUX_CONT),MAX(FLUX_TOT/FLUX_CONT)]  
   ylim=[0.d0,MAX(FLUX_TOT/FLUX_CONT)]     
;   ylim=[0.d0,2.5d0]
ENDIF
;
;
...
ENDIF
;
ytitleStr=textoidl('F_{tot} / F_{c}')
...
;---------------------------EQUIVALENT WIDTHS---------------------------
;
;calculate for default transition-wavelengths, etc.
IF(NOT KEYWORD_SET(lam0)) THEN BEGIN
   lam0=!lam_halpha
ENDIF
;
...
ew_xobs=0.d0
;
equiv_width, flux_tot, flux_cont, xobs, ew_lamb, ew_nue, ew_xobs, /XOBS, lamb0=LAM0, vth_fiducial=VTH_FID*1.d5
;
print, ''
print, 'equivalent widths for'
print, '   lambda=  ', lam0
print, '   vth_fid= ', vth_fid
print, 'lambda-regime: ', ew_lamb
print, 'xobs-regime:   ', ew_xobs
print, 'freq-regime:   ', ew_nue
;
;----------------------PLOT---------------------------------------------
;
